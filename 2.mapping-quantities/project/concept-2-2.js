...
function preload(){
  power = loadTable('data/concept-2-2/mil-power.csv',
  'csv',
  'header');

  spend = loadTable('data/concept-2-2/mil-spend.csv',
  'csv',
  'header');
  personnel = loadTable('data/concept-2-2/mil-personnel.csv',
  'csv',
  'header');

  largeNuc = loadTable('data/concept-2-2/nuclear-largest-test.csv',
  'csv',
  'header');
  noOfNuc = loadTable('data/concept-2-2/nuclear-test-numbers.csv',
  'csv',
  'header');
}
...
function makeObject(x,y,r,c){
  ...
  rect(x,y,objectSize/10,-nucRectSize)

  //spend
  spendSize = rowValsMinMax(spend,r);
  let spendScale  = chroma.scale('YlOrRd').mode('lch');
  console.log(spendSize.max, spendSize.min)
  var spendColorVal = map(spendSize.values[c], spendSize.min, spendSize.max, 0, 1)
  fill(spendScale(spendColorVal).rgb())

  //personnel
  personnelSize = rowValsMinMax(personnel,r);
  personnelRectSize = map(personnelSize.values[c+1], personnelSize.min, personnelSize.max, objectSize/3, objectSize)
  rect(x,y,objectSize/10,personnelRectSize)
